...
export default function ReactCalendar({ events, tags }: Props) {
  ...
  const openEventModal = (event: any, updateURL: boolean) => {
    ...
    if (updateURL) {
      const startParam = event.start
        ...
        : '';
      history.pushState({ eventId: event.id }, '', `?event=${event.id}${startParam}`);
    }
  };
  ...
  const checkForEventInURL = () => {
    ...
    if (eventId) {
      const eventData = transformEvents.find(e => e.id === eventId || e._id === eventId);
  ...
  const initialView = useMemo(() => {
    if (typeof window !== 'undefined') {
      return window.matchMedia('(max-width: 768px)').matches ? 'listMonth' : 'dayGridMonth';
    }
  ...
  return (
    <div id="calendar-wrapper">
      ...
      {modalEvent && (
        <div id="modal-overlay" className="modal-overlay" onClick={closeEventModal}>
          <div id="event-modal" className="event-modal" onClick={e => e.stopPropagation()}>
